...
def problem2b(rect, n, delta, win):
    ...
    for k in range (n+1):
        new_top_left_x = top_left.x - ((1+ k) * delta)
        new_top_left_y = top_left.y - ((1+k)*delta)
        new_lower_right_x = lower_right.x + ((k+1) * delta)
        new_lower_right_y = lower_right.y + ((k+1)*delta)
        rectange = rg.Rectangle(rg.Point(new_top_left_x, new_top_left_y), rg.Point(new_lower_right_x, new_lower_right_y))
        rectange.attach_to(win)
